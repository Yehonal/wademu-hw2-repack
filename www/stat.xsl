...
<xsl:apply-templates/>
  ...
  <xsl:template match="serverload">
	...
	<td><a href="honor.xml" target="_self">Honor</a></td>
  ...
  <xsl:template match="players">
      <div class="center">
      <p class="text"><span class="stat"><xsl:value-of select="count(./*)"/></span> Players Are Now Having Fun.</p>
      </div>
	...
	<xsl:sort select="level" data-type="number" />
	  ...
	  <td class="l3v3l">
            ...
            <xsl:attribute name="class">
               ...
               <xsl:when test="level &lt; 50">mid</xsl:when>
               <xsl:when test="level &lt; 100">high</xsl:when>
               <xsl:when test="level &gt; 200">high-blink</xsl:when>
               </xsl:choose>
			   </xsl:attribute>
	    ...
	    <div>
		 ...
		 <td class="m4p">
			...
			<xsl:when test="map = 34">The Stockades</xsl:when>
			...
			<xsl:when test="map = 90">Gnomeregan Instance</xsl:when>
			...
			<xsl:when test="map = 451">Programmer Island</xsl:when>
		 ...
		 <xsl:attribute name="class">
               ...
               <xsl:when test="plevel = 1">VIP</xsl:when>
               <xsl:when test="plevel = 2">Guard</xsl:when>
               <xsl:when test="plevel = 3">Moderator</xsl:when>
               <xsl:when test="plevel = 4">GM</xsl:when>
               <xsl:when test="plevel = 5">SuperVisor</xsl:when>
               <xsl:when test="plevel = 6">Admin</xsl:when>
               <xsl:when test="plevel = 7">Developer</xsl:when>
               <xsl:when test="plevel = 8">Developer</xsl:when>
	           <xsl:when test="plevel &gt; 8">MainAdmin</xsl:when>
               </xsl:choose>
            </xsl:attribute>
			<xsl:choose>
               <xsl:when test="plevel = 0">Player</xsl:when>
